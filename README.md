...
- Optional persistent sign-in restore across app restarts via signed remember token (`APP_AUTH_SIGNING_KEY`, `APP_AUTH_REMEMBER_DAYS`). Browser-cookie mode is optional and off by default (`APP_AUTH_COOKIE_ENABLED=false`) due Streamlit component compatibility.
...
- auth persistence settings (`APP_REQUIRE_PASSWORD_AUTH`, `APP_AUTH_SIGNING_KEY`, `APP_AUTH_REMEMBER_DAYS`, `APP_AUTH_COOKIE_ENABLED`)
...
- `Your auth accepted URL1` should be something like:
  - `https://auth.goldenstackers.com/ebay/accepted`
- `Your auth declined URL1` should be something like:
  - `https://auth.goldenstackers.com/ebay/declined`
...
- `.env` `EBAY_REDIRECT_URI` is reserved for a future direct callback endpoint implementation.
...
For local testing, use an HTTPS tunnel or hosted HTTPS page for accept/decline URLs (for example, a temporary HTTPS endpoint), then copy the returned `code` into the Streamlit eBay page.
