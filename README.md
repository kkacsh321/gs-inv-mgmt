...
- QuickBooks Online integration blueprint: [QUICKBOOKS_ONLINE_INTEGRATION.md](QUICKBOOKS_ONLINE_INTEGRATION.md).
- Current QuickBooks Online clearing-account foundation (`2026-06-17`): added a direct-integration blueprint and pure payload builders for Intuit QBO v3 `SalesReceipt` and `Purchase` transactions. The design posts gross eBay sales and fee deductions through `Custom App Clearing Account`, uses generic `eBay Sales Customer` and `eBay Vendor` records, keeps eBay marketplace-facilitator tax as `NON`, supports non-order payout deductions such as shipping labels and subscriptions, and validates missing item/account refs before any future live API push.
- Current QuickBooks Online integration strategy (`2026-06-17`): GoldenStackers will duplicate the clean payout-summary behavior of A2X/Link My Books inside this app rather than using a bridge as the production path. Native e-commerce connectors remain out because they tend to create buyer-profile bloat and weak net-payout fee handling. The documented fallback is a monthly manual journal entry from eBay financial summaries plus GoldenStackers report evidence.
- Current QuickBooks Online payload previews (`2026-06-17`): Reports now derives `QuickBooks SalesReceipt Payloads`, `QuickBooks Fee Purchase Payloads`, `QuickBooks Shipping Label Purchase Payloads`, and `QuickBooks Payload Readiness` from the existing QBO sales staging export, exposing endpoint, generated QBO DocNumber, clearing-account/account refs, validation issues, raw JSON payloads, and readiness rollups for accountant/developer review before live API posting is enabled. Shipping-label Purchase previews reduce `Custom App Clearing Account` against the configured eBay Shipping Expense account instead of folding label spend into order-fee rows.
- Current QuickBooks Online runtime settings (`2026-06-17`): Admin > Integrations now exposes review/edit controls for QBO clearing-account refs, generic eBay customer/vendor/payment method refs, fee/shipping/subscription/adjustment expense refs, marketplace-facilitator tax code, and DocNumber prefix. Runtime defaults are seedable, and Reports payload previews now use those DB-backed settings.
- Current QuickBooks Online config health (`2026-06-17`): System Health required-runtime coverage now includes the QuickBooks export toggle plus QBO clearing-account/customer/vendor/payment/account/tax/doc-prefix settings, so missing or inactive QuickBooks posting prerequisites are visible and auto-fixable before live sync work begins.
- Current QuickBooks Online dry-run sync (`2026-06-17`): the `quickbooks_export` sync job now runs as a no-live-write evidence producer. It reads sales actual-economics rows for a configurable lookback window, generates SalesReceipt/order-fee Purchase/shipping-label Purchase payload manifests with deterministic DocNumbers and payload hashes, records sync-run/events/errors, marks review-needed payloads as partial runs, and explicitly blocks `live_post=true` until OAuth, QBO ID resolution, idempotency state, and sandbox validation are implemented.
- Current QuickBooks Online workspace (`2026-06-17`): QuickBooks settings and export controls now live on a dedicated QuickBooks page, including clearing-account refs, sales/shipping income refs, generic eBay customer/vendor/payment method refs, fee/shipping/subscription/adjustment expense refs, marketplace-facilitator tax code, DocNumber prefix, dry-run job enablement/lookback, recent QuickBooks sync runs/events, and a downloadable monthly manual journal fallback worksheet. Admin now points operators to the page while retaining raw runtime setting management.
- Current eBay fee tax-planning evidence (`2026-06-17`): quarterly estimated-tax planning now includes `Quarterly Estimated Tax Fee Summary` and `Quarterly Estimated Tax Fee Detail`, surfacing eBay/marketplace fees separately as advisor-review evidence for potential commissions/fees expense treatment while keeping fees in net-before-COGS and QuickBooks fee Purchase previews.
...
- Current Goldie AI Accountant identity (`2026-05-21`): the AI Accountant now presents as `Goldie` in the dedicated workspace, Ask GoldenStackers, scheduled/in-app monitor messages, Slack command aliases, and default runtime prompts. Operators can ask `Goldie ...` or use `goldie answer ...` in Slack/Ask while existing `ai_accountant_*` audit keys and evidence packet filenames stay stable.
- Current Goldy/Goldie separation (`2026-05-21`): `Goldy` remains the Ask/multi-agent coordinator for routing and orchestration, while `Goldie` is the specialist AI Accountant. Goldy plans can route accounting/tax work to Goldie, but the two identities stay distinct in UI copy, prompts, and docs.
- Current Ask/Goldy chat expansion (`2026-05-21`): the sidebar now lists the accountant workspace as `Goldie`, and Ask GoldenStackers can answer normal conversational prompts through Goldy by default while preserving app-specific snapshot tools. Write/action intent now creates a blocked `goldy` integration-queue approval request instead of directly executing from chat, keeping human approval and action-specific executors between free-form requests and production writes.
- Current Goldie lot-reconciliation context (`2026-05-22`): Ask/Goldie accounting snapshots now include priority P0/P1 exception evidence plus historical lot-assignment evidence up to the snapshot time, with per-lot landed total, explicit assignment total, resolved FIFO total, quantity coverage, source mix, sample assignment rows, and over/underallocation hints so questions such as `lot_overallocated purchase_lot#15` can be answered from source reconciliation numbers instead of a generic no-evidence response.
- Current eBay store-category support (`2026-06-15`): Listings can now manage local eBay store category full paths, sync the seller's current store category hierarchy from eBay Trading API `GetStore`, and apply up to two optional store categories to new eBay listing defaults, Listings publish/revise flows, and Listing Wizard direct-post profiles/payloads. These paths are persisted in listing eBay publish metadata and sent as Inventory API `storeCategoryNames`. Synced rows carry per-category sync status, and operators can opt in to deactivate previously eBay-synced categories that are missing from the latest `GetStore` response while leaving manual categories untouched.
- Current eBay store-category scheduled sync (`2026-06-15`): sync-runner now includes an `ebay_store_categories_sync` job, default enabled with a 24-hour interval, that imports the eBay Store hierarchy through Trading API `GetStore` and records normal sync-run/event evidence. Scheduled stale-category deactivation is off by default; when enabled it uses the same safe rule as the UI and only deactivates previously eBay-imported category rows.
- Current eBay store-category runtime controls (`2026-06-15`): Admin Sync Job Controls and Runtime defaults now expose `ebay_store_categories_sync` enablement, interval, and stale-deactivation settings, and System Health required-runtime coverage includes those keys so missing/inactive settings are visible and auto-fixable.
- Current eBay store-category manual sync evidence (`2026-06-15`): the Listings `Sync From eBay Store` button now executes the same `ebay_store_categories_sync` job used by sync-runner, so manual imports get token-refresh handling plus sync-run, sync-event, and sync-error evidence instead of a separate ad hoc path.
- Current eBay store-category Sync page execution (`2026-06-15`): the Sync workspace execute-now form now exposes eBay marketplace and stale-deactivation controls when `ebay_store_categories_sync` is selected, while avoiding the order-import retry UI for this job until retry controls are fully job-specific.
- Current eBay store-category sync evidence view (`2026-06-16`): Sync run detail now summarizes `ebay_store_categories_sync` event payloads with marketplace, site ID, eBay ack, imported/missing/deactivated counts, and stale-deactivation mode before the raw event table.
- Current eBay store-category freshness signal (`2026-06-16`): the Listings store-category manager now shows the latest `ebay_store_categories_sync` run status, timestamp, processed/updated count, and failure count directly beside the manual sync controls.
- Current lot-allocation monitor materiality (`2026-05-22`): lot over/underallocation exceptions now use an accounting materiality floor instead of a one-cent threshold, include explicit dollar/percentage deltas and correction guidance in the exception details, reserve P0 overallocated severity for gaps that are at least 10% of the lot total or at least $25, keep smaller overallocations visible as P1, and move small underallocations to P2 cleanup so Slack/Goldie question queues prioritize missing COGS and material lot problems. Goldie's monitor and Ask prompts now tell the LLM not to assume the lot total is automatically correct; either assignment costs or lot landed cost evidence may need correction, and the Ask context labels these as unresolved mismatches until an operator confirms the correct side.
- Current Goldie question priority (`2026-05-22`): Goldie monitor and Ask question generation now applies an accounting-specific priority order so missing cost basis, missing product links, dashboard profit-basis review, and nonpositive-margin sales surface before repeated lower-impact lot cleanup questions, while material lot reconciliation still remains visible.
- Current missing-cost-basis evidence (`2026-05-22`): accounting exceptions for missing sale COGS now include sale product/listing/order identifiers, quantity, SKU/title, net before COGS, FIFO evidence summary, and the normal correction path so Goldie can direct operators to verify the sale link and add product-lot assignment landed cost, lot allocation evidence, product landed acquisition cost, or product_cost before close sign-off.
- Current nonpositive-margin evidence (`2026-05-22`): nonpositive-margin accounting exceptions now include the full gross/shipping/fee/label/net-before-COGS/COGS formula, COGS source, FIFO evidence, and review-focus notes for COGS source, fee evidence, label spend, or intentional low-margin sale review so Goldie can separate true pricing losses from bad cost-basis inputs.
- Current dashboard profit-basis question evidence (`2026-05-22`): dashboard profit-basis monitor rows now include review-needed sale IDs and estimate-basis sale IDs from dashboard live metrics, so Goldie can answer "which rows first?" with concrete sale references even when the review context is compacted.
- Current Goldie profit-basis audit context (`2026-05-22`): Ask/Goldie accounting snapshots now include a bounded Profit Basis Audit evidence section from `dashboard_profit_basis_rows`, listing sale ID, SKU, net before COGS, FIFO COGS, profit before returns, COGS source, review severity, evidence-row count, and review reason for review/estimate/nonpositive rows.
- Current Goldie Slack evidence previews (`2026-05-22`): Goldie monitor Slack summaries now append compact evidence previews for dashboard profit-basis review, missing cost basis, nonpositive margin, and material lot reconciliation rows so the first alert shows sale IDs, formulas, or reconciliation deltas instead of only generic recommended actions.
- Current Goldie question evidence hints (`2026-05-22`): Goldie-generated Ask/Slack questions now carry a compact evidence preview and amount beside each reply prompt, so operators can answer questions like `accountant answer missing_cost_basis sale#25:` with the relevant sale/lot evidence visible in the same message.
- Current Goldie scheduler/health labels (`2026-05-22`): sync-runner monitor logs and System Health rows now use Goldie-facing labels for monitor due/disabled/completed/failed states, LLM route status, review evidence, and answer follow-ups while preserving the existing `ai_accountant_*` runtime keys and audit/event names.
- Current Goldie admin/setup copy (`2026-05-22`): Admin runtime defaults, workflow profile descriptions, notification route controls, and Goldie setup checks now use Goldie-facing wording while retaining the stable `ai_accountant_*` setting keys for migrations, audit history, and production compatibility.
- Current Goldie Reports review copy (`2026-05-23`): Reports close-readiness review UI now labels the specialist review as Goldie across the section heading, run button, permission prompt, result expander, outcome buttons, success/error messages, and Copilot/Goldie outcome evidence text while retaining existing `ai_accountant_review` audit metadata.
- Current Goldie Ask/Slack help copy (`2026-05-23`): Ask GoldenStackers permission prompts and Goldy orchestration copy now refer to Goldie as the accounting specialist, and Slack Ops help lists Goldie/accountant commands as read-only Goldie answers while preserving existing command aliases.
- Current Goldie copy sweep (`2026-05-23`): remaining app-facing references now use Goldie wording in Reports prompts, System Health query labels, and Goldie question parsing docs; the only remaining `AI Accountant` strings in app code are intentional role/identity labels or roadmap phase names.
- Current multi-lot FIFO dashboard guard (`2026-05-23`): regression coverage now verifies that a product sold across multiple repurchase lots with explicit assignment costs keeps dashboard profit basis `ok`, marks the COGS as verified, and does not force a review-needed profit warning simply because the sale spans different lot prices.
- Current mixed FIFO fallback review guard (`2026-05-23`): regression coverage now verifies that a sale spanning both explicit assignment cost and ambiguous equal-quantity lot fallback is classified as `mixed_fifo_cost`, counted in dashboard review-needed COGS, and surfaced in Profit Basis Audit with a blended FIFO evidence warning.
- Current mixed FIFO source split (`2026-05-24`): FIFO cost-source classification now separates mixed verified evidence (`mixed_verified_fifo_cost`), mixed expected-quantity estimates (`mixed_estimate_fifo_cost`), and mixed fallback review rows (`mixed_fifo_cost`) so dashboard, Reports, and Goldie do not over-warn on fully evidenced multi-lot sales while still flagging ambiguous equal-fallback or missing-basis blends.
- Current Reports mixed FIFO close evidence (`2026-05-26`): Accounting Close Readiness, close checks, and evidence packet metadata now expose mixed verified, mixed estimate, and mixed fallback COGS separately, with drift checks tying each sub-bucket back to Sold COGS Source Summary so review-needed mixed FIFO no longer obscures fully evidenced mixed-lot sales.
- Current Goldie Ask COGS split context (`2026-05-26`): Ask/Goldie report snapshots now include a sold COGS evidence split line and citation payload for verified, estimated, and review-needed COGS, so chat answers can explain mixed-lot profit basis without reverse-engineering raw source names.
- Current Slack COGS split summaries (`2026-05-26`): scheduled daily Slack reports and manual Admin business-status previews now prepend a verified/estimated/review-needed COGS evidence split ahead of the source mix while preserving existing `{cogs_source_mix_line}` templates.
- Current QBO COGS basis buckets (`2026-05-26`): QuickBooks-style sales and return adjustment staging exports now include `cogs_basis_bucket` alongside existing COGS source/review fields, making verified, estimate, and review rows filterable in accounting exports.
- Current COGS bucket rule centralization (`2026-05-26`): dashboard live metrics, dashboard Profit Basis Audit rows, Reports, Goldie/Ask snapshots, Slack/Admin summaries, and QBO staging exports now share one COGS source bucket helper for verified, estimate, and review-needed classification, reducing drift when source labels change.
- Current accounting formula centralization (`2026-05-26`): Reports, dashboard metrics/fallback rendering, Goldie/Ask snapshots, scheduled Slack summaries, report drift validators, and lot profitability now share canonical helpers for net before COGS, shipping delta, return refund totals, return profit impact, profit before returns, and estimated profit after returns.
- Current fee/shipping evidence impact checks (`2026-05-26`): Reports close readiness, accounting close packet manifests/README guidance, close sign-off evidence/review checks, Goldie/Ask snapshots, scheduled Goldie monitor messages, deterministic Goldie fallback reviews, and Goldie evidence packet summaries now record the dollar impact behind sale-field fee fallback and paid-shipping rows that are missing label spend, so close blockers, operator questions, and accountant handoff packets show both row counts and affected fee/shipping dollars before accounting sign-off.
- Current single-product lot listing COGS (`2026-06-01`): sales, inventory movement, dashboard live metrics, Reports sales rows, and Profit Basis Audit now infer single-product lot quantities from listing titles such as `Lot of 20 ...` when bundle metadata is missing, so one sold eBay listing can consume and cost all 20 inventory units instead of only one.
- Current expanded lot-title quantity inference (`2026-06-09`): the shared listing quantity parser now also recognizes `Full Tube of 20 ...`, `Tube/Roll of N ...`, and conservative `Lot 13 ...` title forms while avoiding `Lot 90% ...` false positives, so dashboard, Reports, FIFO COGS, and sale inventory movements treat one marketplace listing unit as the correct inventory-unit count.
- Current multi-product bundle accounting verification (`2026-06-09`): regression coverage now verifies mixed-product bundle sales with different component FIFO lot costs, multi-listing-unit sales, inventory movement quantities, Profit Basis Audit evidence, dashboard COGS/profit, and one-listing-unit return COGS reversal so bundle profit does not collapse to a single product quantity or product default cost.
- Current bundle/lot quantity labeling (`2026-06-01`): dashboard Profit Basis Audit and Reports close-packet guidance now call out that `quantity_sold` is marketplace listing units while `listing_bundle_inventory_units_sold` is inventory units consumed for bundle/lot COGS, making lot-of-20 sales easier to interpret.
- Current legacy lot-listing reconciliation guard (`2026-06-01`): Accounting Exception Queue now flags sales where an inferred `Lot of N` listing expects more inventory units than the recorded sale movements consumed, and active listing stock checks include inferred single-product lots even when no bundle JSON was saved.
- Current Goldie lot-listing mismatch follow-up (`2026-06-01`): Goldie monitor/action helpers now prioritize `listing_lot_inventory_movement_mismatch` ahead of lower-impact lot cleanup, recommend reconciling sale movements to inferred lot quantities, and ask operators whether the sale should consume the inferred lot quantity or recorded movement quantity.
- Current Reports close guard for lot-listing mismatches (`2026-06-01`): Accounting Exception Queue Checks now includes a named `lot_listing_inventory_movement_mismatches_present` warning, so close readiness and exported close packets explicitly block/review legacy `Lot of N` sales whose movement quantity does not match inferred inventory units.
- Current Ask/Goldie lot-listing mismatch context (`2026-06-01`): Ask/Goldie accounting snapshots now summarize `listing_lot_inventory_movement_mismatch` counts and inventory units needing reconciliation, and include the operator question asking whether the inferred lot quantity or recorded movement quantity is correct.
- Current dashboard lot-listing mismatch warning (`2026-06-01`): dashboard live metrics now publish 30-day lot-listing movement mismatch count/unit totals, and the dashboard warns when legacy `Lot of N` sale movements need reconciliation before close sign-off.
- Current Profit Basis Audit movement evidence (`2026-06-02`): dashboard Profit Basis Audit rows now include expected inventory movement units, recorded inventory movement units, and lot-listing movement mismatch flags/units so legacy `Lot of N` sale drift can be reconciled at the exact sale row.
- Current Reports COGS movement evidence (`2026-06-02`): Reports `COGS & Margin Detail` now includes the same expected/recorded inventory movement unit fields and lot-listing mismatch flags, carrying sale-row movement drift into exports and Accounting Close Packets.
- Current Sold COGS movement drift rollup (`2026-06-02`): `Sold COGS Source Summary` now aggregates lot-listing movement mismatch sale counts and units by FIFO COGS source, making close-packet movement drift visible at both source-summary and sale-detail levels.
- Current COGS source movement drift close guard (`2026-06-02`): Accounting COGS Source Checks now warns on `sold_lot_listing_movement_mismatches_present` when Sold COGS Source Summary contains lot-listing movement drift, so close readiness blocks until legacy lot-of-N sale movement quantities are reconciled.
- Current lot-listing movement repair workflow (`2026-06-02`): Reports now exposes a preview/apply repair action for `listing_lot_inventory_movement_mismatch` rows. The repository repair records only the missing sale movement delta, updates product stock, writes a short audit action compatible with the `audit_logs.action` 32-character limit, refuses no-op repairs, and blocks repairs that would overdraw current stock unless explicitly allowed by a caller.
- Current accounting exception auto-repair workflow (`2026-06-09`): Reports now includes an `Accounting Exception Auto Repair` panel with preview/apply actions for deterministic exception classes: lot under/overallocation can normalize explicit assignment allocations to the lot landed total, `lot_equal_fallback_review_needed` can record the current equal-quantity mixed-lot allocation as explicit assignment weights after operator approval, active bundle listing stock shortages can reduce/end local active listing quantity to current complete-bundle stock, active bundle component overcommit can reduce newest local active bundle listings until component commitments fit stock, and legacy lot-listing movement mismatches can invoke the existing sale movement repair. `nonpositive_margin` remains review-only because repairing a true low-margin/loss sale requires human accounting judgment over sale price, fees, label spend, and cost basis.
- Current accounting repair bulk safety (`2026-06-09`): Reports classifies auto-repair candidates by scope and adds separate `Preview/Apply Cost Repairs` actions for cost-basis-only cleanup. Full `Preview/Apply All Repairs` remains available, but inventory/listing repairs are called out separately because they can change local stock/listing quantities or require marketplace follow-up.
- Current shipping-label finance repair (`2026-06-09`): `unmatched_shipping_label_finance_entry` rows can now be previewed/applied from Reports or the approval-gated accounting queue when the normalized shipping-label finance entry belongs to an order with exactly one sale and that sale has no label spend. The repair copies the finance amount to `sales.shipping_label_cost`, records currency/date evidence where available, writes a short sale audit action, refuses multi-sale orders, and Reports exposes separate `Preview/Apply Finance Repairs` bulk actions so label evidence can be fixed without touching listing or stock quantities.
- Current reconciliation flag visibility (`2026-06-09`): Close Readiness now shows a `Reconciliation Flag Detail` table whenever the `Reconcile Flags` metric is nonzero, and exports the same detail in close packets. Marketplace reconciliation now accepts either `order_total_sum = sales_gross` or `order_total_sum = sales_gross + sales_shipping_cost`, using the closer basis before flagging. The detail table names the marketplace, sales/order counts, sales gross, buyer shipping, order total sum, both deltas, selected basis, and final reconcile delta so expected buyer-shipping differences do not block close.
- Current close blocker visibility (`2026-06-09`): Reports now distinguishes hard P0/reconciliation blockers from accounting close-check warning blockers in the Close Readiness message, renders an inline aggregate `Close Blocker / Warning Detail` table, and adds/export an `Accounting Close Warning Source Detail` table with the exact underlying formula, shipping, COGS, or margin check rows, expected/observed values, deltas, formulas, and sources. Dashboard also escapes currency in the COGS evidence split caption so dollar amounts do not render as Markdown/LaTeX fragments.
- Current close action detail (`2026-06-09`): Reports now adds/export an `Accounting Close Action Detail` table from sale-level COGS & Margin rows for mixed fallback FIFO, lot-listing movement mismatches, and nonpositive margins. The table names sale ID, SKU/title, source, COGS, margin, expected/recorded movement units, mismatch units, issue tags, and recommended actions so close warnings are directly actionable. The Lot Listing Movement Repair panel now uses the same COGS & Margin movement drift rows, merged with exception-queue candidates, so repairs remain available even when exception rows are absent, filtered, or suppressed. Report-side movement matching now falls back through listing product IDs as well as sale product IDs, avoiding false missing-unit rows when the repository repair already sees listing-component movements as matched.
- Current close shipping evidence fallback (`2026-06-09`): Accounting Close Readiness now reads the current `shipping_charged_to_buyer` Shipping Economics column for shipping totals and falls back to Sales/COGS-derived shipping evidence when repository shipping-economics rows are empty or zeroed for a period with sales, including label coverage recalculation, preventing false formula/shipping blockers from zeroed shipping detail.
- Current lot subtotal double-count guard (`2026-06-09`): Accounting exceptions now detect `lot_total_cost_includes_landed_components` when a one-assignment lot item-subtotal field appears to contain the full order total while tax/shipping/handling are also stored separately. Reports/Integration Queue can preview/apply a deterministic repair that reduces `purchase_lots.total_cost` to the item subtotal before assignment allocation repair spreads the separate landed components; Reports orders that repair before generic lot allocation, shows repair-order metadata, and direct allocation repair refuses the high-confidence double-count pattern until subtotal repair runs. The generic underallocation row now names the subtotal repair dependency when this pattern exists. Inventory Intake, Lots, and Search/Edit now label the field as item subtotal before tax/shipping/fees, and purchase-document extraction prefers `subtotal` or derives `total - components` instead of defaulting order total into the subtotal field.
- Current accounting exception suppression workflow (`2026-06-09`): Reports now includes an `Accounting Exception Suppressions` panel so an operator with update permission can mark a row as false positive / no repair needed. Suppressions are stored as reversible audit events keyed by exception type, entity type, and entity id, and active suppressions are filtered out of the shared accounting exception rows used by Reports, dashboard/Goldie rollups, and close review.
- Current active listing bundle quantity source (`2026-06-09`): active listing stock-shortage/overcommit checks and auto-repairs now use explicit bundle/lot quantity metadata saved by Listing Wizard/Listings, not title-only count inference. Title parsing remains a legacy fallback for sold sale movement/COGS reconciliation when old listings lack bundle metadata, and container-weight titles such as `Roll 425 Net Grams` no longer infer 425 inventory units.
- Current eBay order listing-link repair (`2026-06-09`): eBay order import now recovers product/listing identity from `legacyItemId` when eBay sends listing-scoped SKUs, backfills missing sale/order-item product and listing links on repeat sync, and Reports includes a preview/apply `eBay Order Listing Link Backfill` repair for historical rows whose stored eBay order payload can be matched to local listings. Dashboard Profit Basis Audit also falls back through the linked listing product so SKU/title/COGS context no longer goes blank when `sales.product_id` is missing.
- Current approval-gated accounting repair executor (`2026-06-09`): Integration Queue now supports both the legacy `integration=accounting` / `action=repair_sale_lot_listing_movements` payload and the generic `action=repair_accounting_exception` payload with `exception_type`, `entity_id`, `dry_run`, and human approval metadata. The generic executor routes the same deterministic Reports repairs for lot under/overallocation, equal-quantity fallback allocation acceptance, bundle stock shortages, component overcommit, and lot-listing movement mismatches while rejecting review-only classes. It also supports approval-gated `suppress_accounting_exception` / `unsuppress_accounting_exception` actions so Goldie/agents can propose false-positive or no-repair-needed decisions without bypassing human approval or audit history.
- Current purchase document Textract fallback (`2026-06-02`): Inventory Intake and Lots purchase-document uploads now share a best-effort Textract helper and store PDFs/images even when AWS Textract AnalyzeExpense rejects the document with `UnsupportedDocumentException`; unsupported content types such as WEBP are skipped locally before calling AWS. The upload records a skipped/failed extraction summary instead of failing document storage, while LLM/PDF-text extraction can still provide best-effort fields.
- Current wizard simplification scope (`2026-05-27`): next product workflow focus is simplifying Inventory Intake Wizard and Listing Wizard around an AI-first draft/review/apply loop, fewer always-visible form sections, clearer readiness blockers, stronger eBay listing description/item-specifics/category/condition assistance, and Slack-thread workflows for intake, comps, draft listing creation, review questions, and human-approved write actions.
- Next AI/Slack wizard steps (`2026-05-27`): define shared AI draft contracts for product intake and listing drafts, add compact operator review cards with confidence/missing-field prompts, let Slack intake/listing commands create or update drafts without forcing full in-app form navigation, and keep publish/create/update actions behind existing permission and approval gates.
- Current business-agent foundation (`2026-05-27`): GS-V10-022 now has named specialist identities for the Business Chat Room: `Kurt` owns inventory intake drafts and lot/cost-evidence questions, `Murdock` owns listing drafts/eBay-safe sales copy/readiness blockers, `Goldie` remains the accounting specialist, `Scout` is planned for research/pricing, and `Atlas` is planned for business monitoring. Ask GoldenStackers now exposes those specialists in Goldy's orchestration roster, Slack recognizes `kurt` as an intake alias and `murdock`/`listing` as approval-gated listing-draft aliases, and migration `0070_business_agents` seeds default prompts/runtime toggles for the new agent layer.
- Current AI draft contract (`2026-05-27`): Kurt and Murdock now share an `ai_agent_draft` workflow contract with normalized fields, confidence, evidence, warnings, missing questions, proposed actions, source refs, signature, and approval state. Slack AI responses can now carry the draft contract plus an apply plan showing whether the draft is blocked by missing confirmations, pending human approval, or ready for an app-specific apply workflow.
- Current in-app Business Chat Room (`2026-05-27`): GoldenStackers now includes an app-native Business Chat Room where users and agents can post durable coordination messages. Messages are stored as `business_chat_room` audit events with room/thread/sender/source metadata, so Slack can remain integrated as a bridge while the primary agent collaboration history is visible in the application.
- Current Slack-to-room mirroring (`2026-05-27`): Slack Ops Kurt intake and Murdock listing AI runs now best-effort mirror their request summary, draft-contract signature, queue job, Slack channel/thread, intent, and source metadata into the in-app Business Chat Room, making Slack-origin agent work visible in the app without blocking the Slack queue if room recording fails.
- Current Ask-to-room mirroring (`2026-05-27`): Ask GoldenStackers turns now best-effort mirror both the operator prompt and selected agent response into the Business Chat Room, including selected agent, intent, elapsed time, Goldy mode/role, and write-approval status metadata. `business_chat_room_ask_mirror_enabled` defaults on, keeping app-origin and Slack-origin agent conversations in one durable room history.
- Current Business Chat Room routing (`2026-05-27`): app room notes now infer addressed agents from message text and aliases, so prompts such as `Kurt intake this`, `@Murdock draft the listing`, or `Goldie review cost basis` are stored with normalized `directed_to` agent keys. Explicit targets also normalize through the same alias map, giving the next auto-reply/apply workflow a stable routing contract.
- Current dedicated Business Chat Room page (`2026-05-27`): the shared agent room now lives on its own Streamlit page with chat bubbles, roster, attachment upload support for images/videos/PDFs/documents, agent mention routing, and optional AI replies from relevant specialists. Ask GoldenStackers links to this page instead of embedding room expanders, avoiding Streamlit nested-expander issues and making the app chat room the primary live collaboration surface.
- Current agent-to-agent handoff metadata (`2026-05-27`): Business Chat Room agent replies now infer handoff targets from their own response text, so comments like `Scout should comp this` or `Goldie should review cost basis` are saved with both the human and the named specialist in `directed_to`. The chat UI shows those targets inline, making agent-to-agent coordination visible.
- Current bounded agent follow-ups (`2026-05-27`): Business Chat Room AI turns now support one bounded follow-up queue. Initial relevant agents respond first; if an agent names another specialist, that specialist can respond once in the same turn, capped by `business_chat_room_max_agent_replies` to avoid uncontrolled loops.
- Current Business Chat Room approval bridge (`2026-05-27`): when a room message asks for a write/action such as creating, updating, publishing, posting, syncing, or applying changes, the app captures a blocked `business_chat_room/write_action_request` queue job with the room message, target agents, attachments, requester, and approval metadata. Chat still performs no direct writes.
- Current room action visibility (`2026-05-27`): the dedicated Business Chat Room page now shows pending room-origin action requests from the integration queue and lets an operator approve or cancel blocked requests from the same page. Approval queues the request for workflow-specific execution while cancellation closes stale/incorrect requests; both paths record room system messages.
- Current room action queue safety (`2026-05-27`): approved `business_chat_room/write_action_request` jobs now have a safe integration-queue handler. The handler acknowledges the approved request and posts back to the room that no direct write was executed until a workflow-specific executor exists, avoiding unsupported-integration failures.
- Current room action routing (`2026-05-27`): Business Chat Room action requests now include deterministic route metadata (`intake`, `listing`, `accounting`, `pricing`, `business_monitor`, or `general`) plus the recommended workflow handoff, so pending approvals and queue acknowledgements show what workspace/executor should handle the request next.
- Current room workflow handoffs (`2026-05-27`): approved Business Chat Room action requests now save a workflow draft handoff under the recommended workflow key, scoped to `business_chat_room:<queue_job_id>`, preserving prompt, requester, attachments, target agents, route, and approval metadata for review/resume without directly applying changes.
- Current wizard handoff inboxes (`2026-05-27`): Inventory Intake Wizard and Listing Wizard now show active Business Chat Room handoffs routed to their workflow. Operators can inspect the originating room request, queue job, requester, attachment count, recommended next step, and load the prompt into the wizard AI seed context without creating inventory/listings or publishing directly.
- Current handoff review cards (`2026-05-28`): Business Chat Room workflow handoffs now build a review-card field summary from embedded Kurt/Murdock AI draft contracts when present, falling back to safe prompt hints such as product ID, listing ID, quoted title, quantity, and cost. Loading a handoff can prefill obvious wizard session fields for review while preserving the no-direct-write boundary.
- Current room action route transparency (`2026-05-28`): Business Chat Room write/action messages now store and display the inferred route on the originating human message, and the approval-captured system notice names the route, recommended workflow, and next step before anything is queued for workflow execution.
- Current room approval UX (`2026-05-28`): Pending Business Chat Room action requests now use a routed selector with prompt preview, route/workflow/next-step details, and direct links to the target workflow page such as Inventory Intake Wizard, Listing Wizard, Goldie, Tools, or Operations Home.
- Current room draft-card rendering (`2026-05-28`): Slack-origin Kurt/Murdock AI draft contracts are now mirrored into the Business Chat Room metadata and rendered in chat history as proposed fields, confidence/source, missing confirmations, review notes, apply-plan status, signature, and target workflow link.
- Current room-origin draft contracts (`2026-05-28`): Business Chat Room app-origin write/action requests for Kurt intake or Murdock listing now attach a normalized AI-agent draft contract to the approval queue payload using safe prompt hints (product/listing IDs, quoted title, quantity, cost, category guess). Approved workflow handoffs therefore carry the same structured field/missing-question/apply-plan evidence as Slack-origin requests.
- Current workflow handoff contract preservation (`2026-05-28`): Approved Business Chat Room workflow drafts now preserve the normalized AI draft contract and apply plan inside the `business_room_action_handoff_v1` payload, so Inventory Intake Wizard and Listing Wizard handoff inboxes can render the same field/confidence/missing-question evidence after approval.
- Current wizard handoff load audit (`2026-05-28`): Loading a Business Chat Room handoff into Inventory Intake Wizard or Listing Wizard now appends a workflow event with handoff draft ID, queue job, source message, route, draft signature, field count, and prompt-load status, making the operator review step traceable without writing product/listing records.
- Current approval draft preview (`2026-05-28`): The Business Chat Room pending-action selector now renders the selected request's draft contract before approval, showing proposed fields, confidence/source, missing confirmations, review notes, apply-plan status, and workflow link at the approval decision point.
- Current room approval metadata (`2026-05-29`): Business Chat Room approve/cancel transitions now best-effort update the embedded queue payload approval/execution metadata (`approved`, `cancelled`, approver/canceller, timestamp, execution state) in addition to the queue job status, so downstream handoffs no longer carry stale `approval.status=pending` after human action.
- Current wizard handoff review evidence (`2026-05-29`): Inventory Intake Wizard and Listing Wizard handoff inboxes now render missing confirmations, proposed actions, and apply-plan status from the preserved draft contract/apply-plan payload, matching the structured evidence shown in the room approval panel.
- Current wizard handoff lifecycle (`2026-05-29`): Inventory Intake Wizard and Listing Wizard handoff inboxes now include `Mark Handoff Reviewed`, which clears the room-origin workflow draft from the active inbox and records a workflow event with handoff draft ID, queue job, source message, draft signature, field count, and clear result.
- Current room handoff visibility (`2026-05-29`): Business Chat Room now includes an `Active Room Workflow Handoffs` view that aggregates active room-origin workflow drafts for the current user across Inventory Intake Wizard, Listing Wizard, Goldie, Tools, and Operations Home, with counts, prompt preview, queue job, route, attachment count, and direct workflow links.
- Current room-level handoff lifecycle (`2026-05-29`): The Business Chat Room active handoff view now includes `Mark Selected Handoff Reviewed`, allowing operators to clear stale/completed room-origin workflow handoffs from the room itself while recording a workflow event and room system message.
- Current active handoff contract preview (`2026-05-29`): The Business Chat Room active workflow handoff selector now renders preserved draft contracts and apply plans for the selected handoff, so operators can inspect proposed fields, missing confirmations, review notes, and blockers before opening or marking reviewed.
- Current handoff lifecycle helper (`2026-05-29`): Business Chat Room and destination wizard reviewed actions now share the same handoff-close service path, so clearing active workflow drafts records consistent workflow event metadata whether the operator closes the handoff from the room, Inventory Intake Wizard, or Listing Wizard.
- Current room agent workload (`2026-06-02`): Business Chat Room now shows an agent-centric workload summary for Kurt, Murdock, Goldie, Scout, and Atlas, grouping pending approvals, queued actions, failed actions, active handoffs, missing questions, captured answers, attachment counts, latest workflow, and attention status so the room reads like an operating desk instead of separate queues.
- Current room workload answer prompts (`2026-06-02`): the Business Chat Room agent workload summary now surfaces the next targeted Kurt/Murdock answer commands for agents with unresolved handoff questions, so operators can reply from the workload view without drilling into each individual handoff card.
- Current room agent activity (`2026-06-02`): Business Chat Room now includes a per-agent activity digest that groups recent room messages, agent-to-agent mentions, pending action requests, and active workflow handoffs by specialist, making it easier to see what Kurt, Murdock, Goldie, Scout, and Atlas are each handling.
- Current room agent focus (`2026-06-02`): Business Chat Room now includes an Agent Focus panel where an operator can select one specialist and see their attention status, approval/queue/handoff/question counts, latest prompt, next answer commands, and recent activity in one compact view.
- Current room prompt suggestions (`2026-06-02`): Agent Focus now includes suggested Business Chat Room prompts tuned to the selected specialist and current attention state, giving operators ready-to-use phrasing for Kurt intake, Murdock listing readiness, Goldie accounting review, Scout comps/pricing, and Atlas operational prioritization while preserving approval-gated writes.
- Current room prompt board (`2026-06-02`): Business Chat Room now includes a room-wide prompt board that lists suggested next prompts for every specialist at once, so operators can quickly decide whether Kurt, Murdock, Goldie, Scout, or Atlas should take the next turn.
- Current room standup (`2026-06-02`): Business Chat Room now includes a deterministic Room Standup summary with aggregate approval, handoff, question, queue, and failure counts, active-agent priority, and a recommended next room prompt sourced from the prompt board.
- Current room coordination suggestions (`2026-06-02`): Business Chat Room now proposes cross-agent handoff prompts such as Murdock asking Scout for comps before pricing, Kurt asking Goldie to review cost-basis/lot evidence, and Atlas triaging failed or pending room actions, keeping agent-to-agent collaboration visible but still chat/approval driven.
- Current room standup coordination priority (`2026-06-02`): Room Standup now prefers the highest-priority cross-agent coordination prompt when one exists, so the recommended next room prompt can surface Scout/Goldie/Atlas handoffs ahead of generic single-agent prompts.
- Current Kurt cost-basis guardrail (`2026-05-29`): Business Chat Room handoff review cards now classify intake cost hints as explicit product unit cost, whole-lot landed total, assignment landed cost, ambiguous acquisition cost, or missing evidence. Ambiguous generic `cost $X` prompts are flagged before apply so Kurt intake handoffs do not silently blur product cost, lot cost, and assignment cost.
- Current Murdock readiness guardrail (`2026-05-29`): Listing handoff review cards now include a compact readiness checklist for product/link evidence, title, description, category, condition, price/economics, media, and item specifics, with blocker/review/ok status shown in the Listing Wizard inbox and Business Chat Room active handoff view.
- Current agent answer contract (`2026-05-29`): Kurt/Murdock draft contracts now have shared helpers for parsing concise operator replies such as `kurt answer quantity: 20`, applying those answers to draft fields, clearing resolved missing questions, preserving operator/source evidence, and moving changed approved drafts back to pending. Slack ingestion now stores parsed agent-answer metadata on queued commands without directly mutating drafts.
- Current answer-to-handoff bridge (`2026-05-29`): Business Chat Room workflow handoff saving now applies any parsed `ai_agent_answer` or `operator_answers` into the preserved draft contract before the wizard sees it, recomputes the apply plan, and keeps the answer rows in the handoff payload for audit review.
- Current Slack answer safety (`2026-05-29`): Slack Ops execution now treats parsed Kurt/Murdock answer-only commands as evidence capture, not new intake/listing work. It records the answer in the queue response, mirrors it into the Business Chat Room, and exits without creating products, listings, or other writes.
- Current in-app answer capture (`2026-05-29`): Business Chat Room messages like `Murdock answer condition_id: 3000` are now recorded as agent-answer evidence, acknowledged in-room, and explicitly excluded from write/action approval queueing so answer capture does not create or modify app records by itself.
- Current in-app answer-to-handoff apply (`2026-05-29`): When an in-app Kurt/Murdock answer matches an active room-origin handoff for the current user, the Business Chat Room now applies that answer to the handoff draft contract, recomputes its apply plan, and records the outcome in the room acknowledgment while still avoiding product/listing/accounting writes.
- Current Slack answer-to-handoff apply (`2026-05-29`): Slack Ops answer-only commands now use the same evidence-only handoff update path as the in-app room. If a matching active Kurt/Murdock handoff exists for the Slack/app user, the embedded draft contract and apply plan are updated; otherwise the answer is still mirrored as evidence without creating records.
- Current targeted handoff answers (`2026-05-29`): Kurt/Murdock answer syntax now supports explicit targets such as `kurt answer handoff 88 quantity: 20` or `murdock answer draft #321 condition id: 3000`, so operators can update a specific active room-origin handoff instead of relying on the latest matching draft.
- Current answer command prompts (`2026-05-30`): Business Chat Room active handoffs plus Inventory Intake Wizard and Listing Wizard handoff inboxes now show targeted answer command suggestions for missing confirmations, making the new `handoff <queue_job_id>` / `draft <draft_id>` syntax visible at the point of review.
- Current draft-card answer prompts (`2026-05-30`): Business Chat Room AI draft contract cards now also show targeted answer command suggestions when missing confirmations are present, using queue/draft metadata from Slack/app mirrored messages when available.
- Current multi-answer prompts (`2026-05-30`): targeted answer command blocks now list all generated missing-confirmation replies for a handoff or draft card, instead of only the first unresolved field.
- Current cost-basis answer prompts (`2026-05-30`): ambiguous Kurt intake cost hints such as generic `cost $X` now generate targeted clarification commands for `product_unit_cost`, `lot_landed_total`, and `assignment_landed_cost`, helping operators resolve accounting scope before applying intake drafts.
- Current wizard answer evidence (`2026-05-30`): Inventory Intake Wizard and Listing Wizard handoff inboxes now show captured operator answers from the preserved draft contract or handoff payload, including field, answer, source, and actor before the handoff is loaded or reviewed.
- Current Murdock readiness answer prompts (`2026-05-31`): Murdock listing handoffs now generate targeted answer commands from blocker/review readiness gaps even when the draft did not include explicit missing questions, covering product link, title, description, category, condition, price/economics, media, and item-specific readiness.
- Current Slack/room answer help (`2026-05-31`): Business Chat Room help now shows targeted Kurt/Murdock answer syntax, and Slack Ops unsupported/denied command responses include a reusable command reference covering Kurt intake, Murdock listing, Goldie/accounting, comps, status, targeted handoff/draft answers, and the evidence-only safety boundary for answer commands.
- Current answer evidence idempotency (`2026-05-31`): Kurt/Murdock answer application now deduplicates repeated operator answers in draft contracts and room handoff payloads, including duplicate field evidence rows, so Slack retries or repeated app replies do not clutter handoff history while the latest field value remains stable.
- Current room answer visibility (`2026-05-31`): Business Chat Room active workflow handoffs now show captured operator answers merged from both the draft contract and handoff payload, matching the destination wizard inboxes and keeping Slack/app answer evidence visible before opening the target workflow.
- Current room handoff field visibility (`2026-05-31`): Business Chat Room active workflow handoffs now show review-card fields for prompt-hint-only handoffs that do not have an embedded draft contract, keeping product IDs, listing IDs, quoted titles, quantities, costs, and other safe hints visible in the room before opening the target workflow.
- Current room attachment evidence (`2026-06-01`): Business Chat Room active workflow handoffs now show structured attachment evidence from the originating room/Slack request, including filename, kind, content type, size, stored media/document reference, and persistence errors, so operators can verify the files Kurt or Murdock should use before opening the target workflow.
- Current wizard attachment evidence (`2026-06-01`): Inventory Intake Wizard and Listing Wizard handoff inboxes now reuse the same structured Business Chat Room attachment evidence rows, so destination review shows file names, types, stored references, and persistence errors instead of only an attachment count.
- Current shared answer visibility (`2026-06-01`): Business Chat Room, Inventory Intake Wizard, and Listing Wizard now reuse one operator-answer evidence helper, merging and deduping draft-contract and handoff-payload answers so destination workflows no longer hide answers that were captured at a different layer.
- Current wizard answer-prompt visibility (`2026-06-01`): Inventory Intake Wizard and Listing Wizard now show targeted Kurt/Murdock answer command suggestions even when there are no explicit missing-question rows, so cost-basis clarifiers and Murdock readiness fixes remain visible from the destination workflow inboxes.
- Current full handoff answer prompt set (`2026-06-01`): Business Chat Room draft cards, active handoff details, Inventory Intake Wizard, and Listing Wizard now request up to eight targeted answer commands per handoff review, so Murdock readiness prompts for product link, title, description, category, condition, price, media, and item specifics can all be visible together.
- Current room draft-card answer prompts (`2026-06-01`): Business Chat Room AI draft cards now render targeted Kurt/Murdock answer commands even when the draft has no explicit missing-question rows, keeping readiness-derived Murdock prompts visible directly in chat history.
- Current actionable answer examples (`2026-06-01`): Kurt/Murdock targeted answer suggestions now use concrete placeholders for listing cleanup, including sample category/condition IDs, buyer-facing HTML description format, existing media asset IDs, and JSON-style item specifics instead of generic `VALUE` where the expected reply shape matters.
- Current structured item-specific answers (`2026-06-01`): Murdock `item_specifics`/aspect answers sent from Slack or the Business Chat Room are parsed from JSON-style replies or easier `Name=Value; Name=Value` pairs into structured draft field values and displayed back in review evidence as compact JSON, while ordinary HTML descriptions remain plain text.
- Current Slack-friendly item-specific prompts (`2026-06-01`): Murdock item-specific answer suggestions now show the easier `Brand=...; Fineness=...` syntax while still accepting JSON for operators who prefer exact structured replies.
- Future agent memory architecture (`2026-05-27`): pgvector is now scoped as an opt-in future retrieval layer for Business Chat Room agents. Relational tables, workflow drafts, audit logs, listings, products, lots, sales, and accounting evidence remain the source of truth; vector storage would index selected text/evidence chunks for semantic retrieval so Kurt, Murdock, Goldie, Scout, Atlas, and Goldy can remember prior decisions, product/listing patterns, accounting guidance, comp research, Slack/app threads, and operator preferences without treating embedding hits as authoritative records.
- Current eBay fee estimate tool (`2026-05-21`): Tools now includes an `eBay Fee Calculator` tab for estimating final value fees, fixed per-order fees, promoted listing fees, surcharges/upgrades, shipping label spend, packaging cost, item COGS, net before COGS, estimated profit, margin, and breakeven sale price. The shared Listing Wizard/Listings estimator now uses the same managed-payments-style basis and no longer assumes a separate payment-processing fee by default, while preserving explicit runtime overrides for any additional surcharge modeling. Admin Go-Live Evidence now exposes editable eBay fee estimate defaults that feed Tools, Listing Wizard, and Listings without code changes, and migration `0068_ebay_fee_defaults` seeds those defaults for existing environments. Rates stay editable because eBay fee treatment varies by category, store subscription, seller performance, listing upgrades, ads, and order details; actual marketplace finance imports remain the accounting source of truth after sale.
- Current pre-publish breakeven signal (`2026-05-21`): Listing Wizard and Listings expected-net panels now show estimated breakeven listing price and price cushion beside expected net/margin, using fee assumptions, buyer shipping, known COGS, and local fulfillment cost so thin or negative listings are visible before draft creation or publish.
- Current expected-net helper alignment (`2026-05-21`): pre-publish expected-net, margin band, breakeven, and price-cushion math now lives in the shared eBay fee-estimator service and is reused by Listing Wizard and Listings, reducing drift between draft creation and publish review economics.
- Current listing helper test isolation (`2026-05-21`): Listing Wizard/Listings helper tests now bootstrap `app.components.views` with the real views package path, removing order-dependent import failures when later tests import normal view modules such as Tools.
- Current eBay fee calculator inventory handoff (`2026-05-21`): Tools eBay Fee Calculator can now search recent/matching inventory products and apply the selected product's known unit cost into the COGS input, preferring explicit `product_cost` and falling back to landed acquisition fields.
- Current listing COGS estimate helper alignment (`2026-05-21`): product known-unit-cost resolution now lives in the shared eBay fee-estimator service and is reused by Tools, Listing Wizard, and Listings, consistently preferring `product_cost` before landed acquisition fallback.
- Current duplicate listing workflow (`2026-06-09`): Listings now supports duplicating an existing listing into a fresh draft from the selected-listing panel or Listing Media Manager. Duplicate drafts preserve reusable setup metadata such as category, policy, bundle, and listing details while clearing eBay external IDs, URLs, offer IDs, publish history, stale errors, lifecycle/review history, and inventory SKU identity so the duplicate can be posted as a separate marketplace listing instead of revising the original. Listing Media Manager can also attach currently unlinked product media to a draft without uploading duplicate files.
...
- Current dashboard/report COGS evidence split (`2026-05-21`): dashboard live metrics now split 30-day sold COGS into verified, estimated, and review-needed dollars, with sale IDs for estimated/review buckets. The dashboard Profit Basis Audit flags each sale as ok/estimate/review with a reason, AI Accountant context carries the split, Reports close readiness shows the same verified/estimated/review-needed sold COGS buckets, and Reports drift checks verify the split ties back to dashboard COGS and profit-basis status.
...
- Current AI Accountant workspace (`2026-05-06`): added a dedicated `AI Accountant` page with role-gated monitor rows from the accounting exception queue and dashboard profit-basis status, grouped action summaries including unresolved-question follow-up rows, recommended cleanup actions, in-app AI Accountant message audit events, recent message history with requested/effective monitor severity metadata, visible question-status columns, unresolved-question warnings, and fallback warnings, retry-safe Slack alert queuing through the notification outbox, a dedicated `notification_route_ai_accountant_monitor` routing control, scheduled/in-app monitor messages with question status counts, System Health visibility for open answer follow-ups, downloadable evidence packets including captured Ask/Slack answer, answer-follow-up evidence, message question-status counts, and answer status counts, visible evidence-packet question/answer status counts and warnings, visible answer-status counts/warnings in the Recent AI Accountant Answers panel, a one-click recommended automation setup action including monitor timezone/local-time/lookback and web-research limit/timeout defaults, runtime visibility for web-research limit/timeout settings, and a read-only LLM monitor review with source citations, prompt/data-scope hashes, structured JSON rendering, deterministic monitor-review fallback when all LLM runtimes fail, accepted/edited/rejected outcome tracking, visible recent review outcome history, and a latest-outcome follow-up banner for edited/rejected reviews.
...
- Current Taxes workspace split (`2026-06-08`): a dedicated `Taxes` sidebar page now hosts the tax-focused workspace instructions and reuses the existing tax reporting engine in tax-workspace mode for Colorado SUTS, Tax Review Packets, tax sign-off evidence, and quarterly estimated-tax planning. Reports now points operators to the Taxes page for SUTS and estimated-tax work while preserving the underlying exports and evidence hashes during the extraction. The normal Reports page now hides tax scope controls, quarterly estimated-tax planning, Tax Drilldown/SUTS upload, Tax Review Packet/sign-off actions, and tax/quarterly evidence tables; the Taxes page keeps those visible, filters its report-table list to tax evidence, suppresses unrelated accounting close, eBay fee, exception queue repair, economics intelligence, purchase-document audit, Reports Copilot, document-handoff, and rebuy widgets, and records workspace feedback under the `taxes` key. Shared SUTS constants, SUTS template/jurisdiction loading, SUTS scope summaries, SUTS workbook generation/warnings, month filtering, CSV scope parsing/default non-facilitator marketplace scoping, Tax Reporting Scope control resolution, tax drilldown filters/KPIs/sale options, tax exception/advisor-review row builders, tax profile/sign-off/payment audit row normalizers, tax report presets, tax reporting sign-off payload/review helpers, quarterly estimated-tax period/payload/payment-review helpers, Taxes workspace intro/help rendering, Tax Review Packet prefix definitions, Tax Review Packet ZIP assembly, and tax packet evidence hashing now live in `app/components/views/tax_support.py` as the first physical extraction out of the Reports module.
- Current quarterly estimated-tax planning progress (`2026-06-08`): Taxes now includes a `Quarterly Estimated Tax Planning` worksheet using IRS estimated-payment income windows (Q1 Jan-Mar, Q2 Apr-May, Q3 Jun-Aug, Q4 Sep-Dec), federal/Colorado due-date references, configurable federal income-tax, Colorado income-tax, self-employment tax, SE net-earnings multiplier, prior-payment, other-income, and deductible-adjustment inputs, and an XLSX packet with summary, payment worksheet, advisor checklist, sales detail, COGS detail, return adjustments, and local tax detail. The packet uses actual fee/label/shipping economics plus FIFO COGS/margin evidence, supports spouse-owned LLC/default partnership versus single-member/S-corp/C-corp/advisor-custom tax-treatment planning choices so SE tax can be included or excluded by advisor direction, defaults owner/spouse allocation to 50/50 and computes each spouse/member SE Social Security taxable earnings against that person's W-2 Social Security wages and wage-base remaining while keeping Medicare separate, flags missing/review-needed/estimated COGS row counts and dollar amounts from the shared COGS basis buckets, shows an in-app scope notice when the IRS quarterly income window differs from the selected report range, includes local/SUTS tax context for the selected scope, records audit-backed quarterly estimated-tax payment evidence with packet hash/confirmation/evidence links, reviews recorded paid amounts, remaining amount, confirmation/evidence-link coverage, and packet-hash coverage against the worksheet by jurisdiction, bundles quarterly worksheets/payment evidence into the Tax Review Packet ZIP and evidence hash, and keeps all outputs clearly labeled as tax-advisor-review planning estimates.
...
- Current accounting repair/review workflow (`2026-06-09`): Reports distinguishes deterministic repairs from human review decisions. Lot/tube listing movement repairs can optionally preserve current inventory, which backfills historical sale movement evidence without reducing today's stock when a legacy sale already depleted or reconciled inventory. Reports Close Action Detail and Dashboard Profit Basis Review Actions can also record audited decisions for accepted nonpositive-margin or mixed-FIFO rows; these clear unresolved warning counts without changing sale economics or COGS evidence.
...
- Orders now roll up marketplace buyers into a `customers` table keyed by eBay username/email/ship-to fallback, link `orders.customer_id`, and expose repeat-buyer order count/lifetime spend in Orders and Slack order-import notifications.
- Orders includes a `Backfill Customers From Existing Orders` action, and migration `0071_customers` backfills historical order rows during deployment.
- Added a dedicated `Customers` page for buyer lookup, repeat-buyer filtering, customer CSV export, and selected-customer purchase history/status review.
- Customers detail now includes per-customer order status breakdown plus compact purchased-item summaries for fast repeat-buyer review.
- Customers detail now includes editable internal customer notes with audit-backed saves.
- Customers directory now includes internal note previews, note search, and `Has notes` / `No notes` filters.
- Customers directory now adds `Recent`, `Warm`, `Dormant 90d+`, and `No orders` follow-up status filters based on each buyer's last linked order.
- Customers detail now shows a compact contact/address summary and searchable/status-filtered purchase history for support and repeat-buyer lookup.
- Orders now surfaces linked customer lifetime spend, internal customer note previews, and repeat-buyer context in the side panel and Orders Copilot evidence.
- Slack eBay order-import alerts now include a bounded internal customer note line when the linked buyer profile has notes.
- Orders filters now include buyer type and customer-note filters so fulfillment can quickly isolate repeat buyers or buyers with internal notes.
- Orders side panel now supports editing linked internal customer notes without leaving the order workflow.
- Ask GoldenStackers/Goldy now has a read-only `customers` domain snapshot for repeat-buyer, customer-note presence, dormant-buyer, lifetime-spend, and top-customer questions.
- Slack Ops now accepts read-only `customer`, `customers`, and `repeat-buyer` intents backed by the same customer snapshot and AI summary path.
- Ask GoldenStackers fallback help now suggests repeat-buyer/customer-note questions so the customer domain is discoverable from general chat.
- Migration `0072_customer_chat_defaults` enables the `customers` Ask domain for ops/admin runtime settings and seeds Slack customer-intelligence prompts/toggles so existing environments get the new customer chat path by default.
- Atlas and Goldie Business Chat Room scopes now include the `customers` domain so repeat-buyer context can participate in operations monitoring and accounting triage.
- Business Chat Room context snapshots now include bounded customer rollups for agent replies: customer count, repeat-buyer count, note-presence count, dormant-buyer count, and top repeat buyers without exposing internal note text.
- Business Chat Room agent prompts now include that bounded customer context, allowing Atlas/Goldie to reason about repeat-buyer follow-up and customer-aware accounting/ops priorities without seeing internal note bodies.
- Business Chat Room now surfaces a `Customer Context` expander with repeat-buyer KPIs and top repeat-buyer summaries, matching the safe note-free context agents receive.
- Business Chat Room prompt board and coordination suggestions now use customer rollups to recommend Atlas repeat-buyer follow-up review and Goldie accounting/tax-sensitive boundary review when repeat buyers or customer-note flags are present.
- Business Chat Room customer rollups now include bounded dormant-customer summaries in both agent prompt context and the operator-facing Customer Context table.
- Business Chat Room Customer Context now links directly to the Customers workspace and shows safe suggested Atlas/Goldie follow-up prompts for repeat-buyer and dormant-customer review.
- Business Chat Room suggested prompts can now be staged into an editable prepared-prompt box from Customer Context, prompt board, standup, coordination, and Agent Focus panels before sending through the normal approval-gated chat path.
...
- Business Chat Room prepared prompts now retain their source label in the sent message metadata/history, so staged Customer Context, Standup, coordination, Focus, and answer-command prompts remain traceable after operator review.
- Business Chat Room prepared prompts now show a pre-send read-only versus write/action approval warning so operators know whether sending will queue a human-approved workflow request.
- Business Chat Room prepared prompts now show pending attachment previews before send, making it clear which images, PDFs, videos, CSVs, or other evidence will be attached to the staged message.
- Business Chat Room chat history now renders sent attachment evidence as a compact table with kind, stored reference, and error fields instead of forcing operators to read raw JSON.
- Business Chat Room attachment evidence rendering is now shared between chat history and active handoff review, keeping file evidence columns consistent across room surfaces.
...
- Business Chat Room approval-required prepared prompt history now includes the routed action/workflow beside the status, making queued approval paths easier to audit.
...
- Admin now includes an `eBay Buyer Blocks` tab with a local blocked-buyer mirror, username normalization/deduping, add/remove/replace workflows, CSV download/copy output, audit events, internal notes, customer/order context for matched blocked buyers, candidate suggestions from unblocked eBay customer records, direct links to eBay Buyer Management pages for applying the live marketplace block list, and a Sell Account user-preferences API capability/smoke-test panel that clearly separates supported seller-preference APIs from the manual blocked-buyer list.
